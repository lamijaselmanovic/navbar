...
function removeExcessItems(screenWidth, items, cutOff) {
  ...
  const sumOfWidths = cutOffItems.reduce(
    (widthSum, { width }) => widthSum + width,
    0
  );
...
class NavBar extends Component {
  ...
  componentDidMount() {
    ...
    // TODO: when component mounts, do this from onWindowResize
  }
  ...
  onWindowResize(event) {
    ...
    const items = this.state.items.map(function(item) {
      const width = item.ref.current
        ? item.ref.current.offsetWidth
        : item.width;
      sumOfWidths += width;
      return {
        ...item,
